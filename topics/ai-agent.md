---
id: topic_ai-agent
title: "AI Agent"
created: 2026-06-28
updated: 2026-06-28
type: topic
tags: [hermes, topic, knowledge]
links:
  - entities/ai-agent发展趋势与工作流革命
  - entities/hermes-agent-搜索抓取工具汇总
  - entities/the-user-has-provided-a-detailed-outline
  - skill:DeepSeek
  - skill:Gemma
  - skill:Google AI
  - skill:MoE Inference
  - skill:Provider: google
session_count: 3
---

# AI Agent

> **Knowledge Layer — 自动生成的主题节点**

## 概述

本主题自动从 Hermes 会话数据中提取生成。
关联 3 个会话，涵盖以下技能和实体。

## 关联技能 (Skills)

  - [[DeepSeek]]
  - [[Gemma]]
  - [[Google AI]]
  - [[MoE Inference]]
  - [[Provider: google]]

## 关联会话 (Sessions)

  - [[entities/ai-agent发展趋势与工作流革命|ai-agent发展趋势与工作流革命]]
  - [[entities/hermes-agent-搜索抓取工具汇总|hermes-agent-搜索抓取工具汇总]]
  - [[entities/the-user-has-provided-a-detailed-outline|the-user-has-provided-a-detailed-outline]]

## 来源

- 数据源: Hermes state.db (SQLite)
- 生成管线: llm-wiki-pipeline.py v2
- 生成时间: 2026-06-28

> 编辑此页面以完善知识内容并添加更多 [[SCHEMA|交叉引用]]。
