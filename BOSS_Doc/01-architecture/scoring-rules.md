...
> **价值奖励 v2.2 (2026-08-08)**：新增第 6 个加权维度，目的把对【投资/全球局势/金融市场】有价值的文章升入 B/A 级（获得 LLM 深度增强），解决评分保守致高价值文章被淹没（原 A=0/B=4.6%）。奖励权重可经配置中心「评分」Tab 覆盖（`value.reward_*`）。实测 5000 篇：A 0→53(1.1%)、B 232→292(5.8%)，B 级入选 Anthropic $10B 交易/Nvidia/半导体征税/Bitcoin/太空股等高价值文章。
