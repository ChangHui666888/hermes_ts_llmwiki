...
- **✅ 全链路统一北京时间 (2026-08-15, commit 30f4d67)**: 规范时区从 **UTC 改为北京时间(Asia/Shanghai, UTC+8)**。scanner `_parse_date` 源时区日期→转北京 naive；aggregator/scorer/event_normalizer 用 `news_intel/timeutil.py` 的 `beijing_now()`/`to_beijing_naive()`（aware 统一剥 tzinfo 防混算 TypeError）；事件 `first_seen/last_updated/event_id` 全北京。VPS 后端 `database.py` 会话时区 `Asia/Shanghai` + docker-compose `TZ` + `dashboard_v1/rss_sources` now 统一北京。前端 `src/lib/time.ts` 把 naive 按北京解析、`timeZone:Asia/Shanghai` 渲染（6 处 timeAgo + IntelligenceFeed + config/stories 全部接入）。存量数据 `scripts/migrate_tz_beijing.py` +8h 迁移（跳过损坏/日期-only）；auto-pipeline 云同步窗口改 `julianday()` 数值比较。
